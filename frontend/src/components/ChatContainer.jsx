    ...
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      ...
      <div className="flex flex-col h-[calc(100% - 120px)] overflow-y-scroll p-3 pb-32">
        {messages.map((message, index) => (
          <div key={index} className={`flex items-end gap-2 justify-end overfow-hidden ${message.senderId !== authUser._id && 'flex-row-reverse'}`}>
            {message.image ? (
            ...
            <div className="text-center text-xs">
                <img src={message.senderId === authUser._id ? authUser?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.profile_martin} alt="" className="w-7 rounded-full"/>
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3">
        <div className="flex-1 flex items-center bg-gray-100/12 px-3 rounded-full">
          <input onChange={(e) => setInput(e.target.value)}  value={input} onKeyDown={(e) => e.key === "Enter" ? handleSendMessage(e) : null} type="text" className="flex-1 text-sm p-3 border-none rounded-lg outline-none" placeholder="Send a message"/>
          <input onChange={(e) => handleSendImage(e)}  type="file" id="image" accept="image/png, image/jpeg" hidden/>
          <label htmlFor="image">
            <img src={assets.gallery_icon} alt="" className="w-5 mr-2 cursor-pointer"/>
          </label>
        </div>
        <img onClick={(e) => handleSendMessage(e)} src={assets.send_button} alt=""/>
      </div>
    </div>
  ) : (
    <div className="flex flex-col justify-center items-center text-gray-500 bg-white/10 max-md:hidden">
